...
twoway 
	...
	title("{fontface Merriweather Bold: Excess Years of Potential Life Lost Rate Among Black People by Age}", size(small))
	legend(order(1 "Black Males" 2 "Black Females" )
		size(small) position(2) ring(0) 
		region(fcolor(white%30)) bmargin(small) 
		title("")) 
	text(3000 50 "↑ Higher rates relative to White people", placement(e) justification(center) size(vsmall))
	text(-2000 50 "↓ Lower rates relative to White people", placement(e) justification(center) size(vsmall))
	xsize(5) ysize(4)
...
twoway 
	...
	scheme(plotplainblind) 
	;
	graph export figure_ratio_excess_ypll_per_100k_over_age_by_sex.jpg, as(jpg) name("Graph") quality(100) replace
	;
#delimit cr
